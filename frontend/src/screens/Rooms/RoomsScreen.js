...
export default function RoomsScreen(props) {
 ...
 const renderDevice = ({ item }) => (
  <TouchableOpacity style={styles.ItemContainer} 
    onPress={subject==='Conditions'? () => navigation.navigate('Conditions', { location: item.id,name: item.name, email, subject }) : () => navigation.navigate('Analysis', { location: item.id, name: item.name,email, subject })}>
    <ImageBackground
    ...
    <View style={styles.Viewcontainer}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.centeredText}>{subject=='Conditions'? 'Điều kiện': 'Phân tích'}</Text>
        </View>
        <FlatList
